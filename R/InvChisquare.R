...
#' distribution with \code{df} degrees of freedom and
#' optional parameter \code{scale}. The parameterization is
#' consistent with Gelman et al. (2013). By default, the
#' \code{scale} is set to return results for the inverse
#' chi-square distribution. If \code{scale} is changed, then
#' the results are returned for the scaled inverse
#' chi-squared distribution.
#'
#' If \code{scale} is omitted, it assumes the default value
#' of \code{1/df}.
#'
#' The scaled inverse chi-square distribution with
#' parameters \code{df = n} and \code{scale = s^2} has
#' density
#'
#' \deqn{f(x)= (2^{-n/2}/Gamma(n/2)s^n x^(-n/2 - 1) e^(-n
#' s^2/(2x))}
#'
#' for \eqn{x \ge 0}, \eqn{n > 0} and \eqn{s > 0}. (Here
#' \eqn{Gamma(n/2)} is the function implemented by R's
#' \code{gamma()} and defined in its help. Note that \eqn{n
#' = 0} corresponds to the trivial distribution with all
#' mass at point 0.)
#'
#' The mean and variance are \eqn{E(X) = n/(n - 2) s^2} for
#' \eqn{n > 2} and \eqn{Var(X) = 2n^2/(n-2)^2/(n-4) s^4} for
#' \eqn{n>4}.
#'
...
#' @return \code{dinvchisq} gives the density,
#' \code{pinvchisq} gives the distribution function,
#' \code{qinvchisq} gives the quantile function, and
...
#' \code{rinvchisq}, and is the maximum of the lengths of
#' the numerical arguments for the other functions.
#'
#' The numerical arguments other than \code{n} are recycled
